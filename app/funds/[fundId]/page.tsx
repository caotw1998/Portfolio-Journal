        ...
        <div className="grid min-w-0 gap-5">
          <div id="performance" className="grid scroll-mt-20 gap-5 lg:scroll-mt-6">
            ...
            <section className="min-w-0 overflow-hidden border border-border bg-card p-4 lg:p-6">
              ...
              <div className="mt-2 min-w-0"><DetailPerformanceChart primaryId={fund.id} primaryKind="fund" primaryName={fund.name ?? fund.code} primaryMarket={fund.market} points={fund.navSnapshots.map((point) => { const valuationDate = date(point.valuationDate); return { date: valuationDate, unitNav: point.unitNav, accumulatedNav: point.accumulatedNav, dailyReturn: point.dailyReturn, dividendAmount: dividendsByDate.get(valuationDate) ?? 0 }; })} dividendEvents={fund.dividendEvents.flatMap((event) => event.exDate && event.amount ? [{ recordDate: event.recordDate ? date(event.recordDate) : null, exDate: date(event.exDate), paymentDate: event.paymentDate ? date(event.paymentDate) : null, amount: event.amount }] : [])} managerTenures={fund.managerTenures.map((manager) => ({ managerName: manager.managerName, startDate: manager.startDate ? date(manager.startDate) : null, endDate: manager.endDate ? date(manager.endDate) : null }))} baselines={baselines.filter((item) => !(item.kind === "fund" && item.id === fund.id))} chartSingleColor={user.chartSingleColor} chartSeriesColors={user.chartSeriesColors} /></div>
            </section>
          </div>
