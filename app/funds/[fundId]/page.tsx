...
import { getEtfPcfSnapshot, getFundDetail, getFundPortfolioReport } from "@/lib/funds/service";
  ...
  return (
    <AppShell currentPath={`/funds/${fund.id}`}>
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[10rem_minmax(0,1fr)] layout-mobile:grid-cols-1 layout-desktop:grid-cols-[10rem_minmax(0,1fr)]">
        <FundSectionNav ariaLabel="基金详情页内导航" items={[{ id: "performance", label: "业绩风险" }, { id: "capital", label: "规模资金" }, { id: "portfolio", label: "持仓变化" }, { id: "quality", label: "数据质量" }]} />
          ...
          <div id="performance" className="grid scroll-mt-20 gap-5 lg:scroll-mt-6">
            ...
            <section className="min-w-0 overflow-hidden border border-border bg-card p-4 lg:p-6">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0"><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">NAV / Total Return</p><h2 className="mt-1 text-xl font-semibold lg:text-2xl">业绩与风险</h2></div>
              ...
              <div className="mt-3 min-w-0"><FundNavChart fundId={fund.id} points={fund.navSnapshots.map((point) => { const valuationDate = date(point.valuationDate); return { date: valuationDate, unitNav: point.unitNav, accumulatedNav: point.accumulatedNav, dailyReturn: point.dailyReturn, dividendAmount: dividendsByDate.get(valuationDate) ?? 0 }; })} managerTenures={fund.managerTenures.map((manager) => ({ managerName: manager.managerName, startDate: manager.startDate ? date(manager.startDate) : null, endDate: manager.endDate ? date(manager.endDate) : null }))} benchmarks={benchmarks.filter((item) => item.status === "active" && item.pointCount > 0).map((item) => ({ id: item.id, name: item.name, code: item.code, market: item.market, dataBasis: item.dataBasis, dataBasisLabel: item.dataBasisLabel }))} chartSingleColor={user.chartSingleColor} chartSeriesColors={user.chartSeriesColors} /></div>
            </section>
          </div>
