...
import { FundCapitalHistory, FundPortfolioHistory, FundSyncButton } from "@/components/fund-detail-interactions";
import { FundNavChart } from "@/components/fund-nav-chart";
...
import { getFundDetail, getFundPortfolioReport } from "@/lib/funds/service";
...
export default async function FundPage({ params, searchParams }: { params: Promise<{ fundId: string }>; searchParams: Promise<{ report?: string }> }) {
  ...
  return (
    <AppShell currentPath={`/funds/${fund.id}`}>
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[10rem_minmax(0,1fr)]">
        <FundSectionNav items={[{ id: "performance", label: "业绩风险" }, { id: "capital", label: "规模资金" }, { id: "portfolio", label: "持仓变化" }, { id: "quality", label: "数据质量" }]} />
        <div className="grid min-w-0 gap-5">
          <div id="performance" className="grid scroll-mt-20 gap-5 lg:scroll-mt-6">

            <section className="border border-border bg-card p-5 lg:p-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">NAV / Total Return</p><h2 className="mt-1 text-2xl font-semibold">业绩与风险</h2></div><p className="font-mono text-xs text-muted-foreground">分红再投资复权 · {fund.navSnapshots.length} 点 · {fund.dividendEvents.length} 次分红</p></div><dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 border-y border-border py-3 text-sm sm:grid-cols-5">{[["近一年", metrics.oneYearReturn], ["近三年", metrics.threeYearReturn], ["年化收益", metrics.annualizedReturn], ["最大回撤", metrics.maxDrawdown], ["夏普", metrics.sharpe]].map(([label, value]) => <div key={String(label)} className="flex items-baseline justify-between gap-2 sm:block"><dt className="text-xs text-muted-foreground">{label}</dt><dd className="font-semibold sm:mt-1">{label === "夏普" ? number(value as number | null) : rate(value as number | null)}</dd></div>)}</dl><div className="mt-4"><FundNavChart fundId={fund.id} points={fund.navSnapshots.map((point) => { const valuationDate = date(point.valuationDate); return { date: valuationDate, unitNav: point.unitNav, accumulatedNav: point.accumulatedNav, dailyReturn: point.dailyReturn, dividendAmount: dividendsByDate.get(valuationDate) ?? 0 }; })} managerTenures={fund.managerTenures.map((manager) => ({ managerName: manager.managerName, startDate: manager.startDate ? date(manager.startDate) : null, endDate: manager.endDate ? date(manager.endDate) : null }))} benchmarks={benchmarks.filter((item) => item.status === "active" && item.pointCount > 0).map((item) => ({ id: item.id, name: item.name, code: item.code, market: item.market }))} chartSingleColor={user.chartSingleColor} chartSeriesColors={user.chartSeriesColors} /></div></section>
          </div>
      ...
      <FundPortfolioHistory fundId={fund.id} reports={fund.portfolioReports} selectedReport={selectedReport} />

      <div className="grid gap-5 lg:grid-cols-2">
        <section className="border border-border bg-card p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Profile · Core</p><h2 className="mt-1 text-2xl font-semibold">基金档案</h2>
        ...
        <section className="border border-border bg-card p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Managers · Full History</p><h2 className="mt-1 text-2xl font-semibold">基金经理</h2>
