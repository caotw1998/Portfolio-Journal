...
export function BenchmarkManager({
  ...
  function clearSelectedBenchmarks() {
    setSelectedIds([]);
    setComparison({
      series: [],
      comparisonRange: null,
  ...
  const activeBenchmarks = benchmarkList.filter(
    (benchmark) => benchmark.status === "active",
  );
  const activeBenchmarkIds = activeBenchmarks.map((benchmark) => benchmark.id);
  const allActiveBenchmarksSelected =
    activeBenchmarkIds.length > 0 &&
    activeBenchmarkIds.every((benchmarkId) => selectedIds.includes(benchmarkId));

  function toggleAllActiveBenchmarks() {
    if (allActiveBenchmarksSelected) {
      clearSelectedBenchmarks();
      return;
    }

    setSelectedIds(activeBenchmarkIds);
  }

  return (
    <div className="grid min-w-0 gap-5">
      <div className="grid min-w-0 gap-5 xl:grid-cols-[0.9fr_1.1fr]">
        <section className="min-w-0 rounded-[1.5rem] border border-border bg-card p-6">
                  ...
                  <div className="flex min-w-0 gap-3">
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <p className="text-base font-semibold">{benchmark.name}</p>
      ...
      <section className="min-w-0 overflow-hidden rounded-[1.5rem] border border-border bg-card p-6">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3">
          {activeBenchmarks.length > 0 ? (
            <button
              type="button"
              onClick={toggleAllActiveBenchmarks}
              disabled={isPending}
              className="rounded-full border border-accent bg-accent px-3 py-2 text-sm font-medium text-accent-foreground disabled:cursor-not-allowed disabled:opacity-60"
            >
              {allActiveBenchmarksSelected ? "清空" : "全选"}
            </button>
          ) : null}
          ...
          {activeBenchmarks.map((benchmark) => (
            ...
            >
              <input
                ...
                onChange={() => toggleSelectedBenchmark(benchmark.id)}
              />
