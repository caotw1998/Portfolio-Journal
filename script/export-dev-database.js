...
async function main() {
  ...
  const [users, funds, userFunds, fundNavSnapshots, fundManagerTenures, fundPortfolioReports, fundHoldings, fundScaleSnapshots, fundSourceSnapshots, benchmarkInstruments, benchmarkPriceSnapshots, benchmarkValuationSnapshots, auditLogs] = await Promise.all([
    ...
    prisma.benchmarkValuationSnapshot.findMany({ orderBy: { date: "asc" } }),
    prisma.auditLog.findMany({ orderBy: { createdAt: "asc" } }),
  ]);
  const snapshot = { version: 2, kind: "fund-research", exportedAt: new Date().toISOString(), tables: { users, funds, userFunds, fundNavSnapshots, fundManagerTenures, fundPortfolioReports, fundHoldings, fundScaleSnapshots, fundSourceSnapshots, benchmarkInstruments, benchmarkPriceSnapshots, benchmarkValuationSnapshots, auditLogs } };
